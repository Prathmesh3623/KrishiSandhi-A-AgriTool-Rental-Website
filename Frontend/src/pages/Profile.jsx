...
export default function Profile() {
    ...
    if (loading) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', fontSize: '1.5rem', color: 'var(--primary-green)' }}>Loading...</div>;
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
                    ...
                    <div>
                        <div style={{ background: 'white', borderRadius: '20px', padding: '25px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', marginBottom: '30px' }}>
                            <h3 style={{ borderBottom: '2px solid #f0f0f0', paddingBottom: '10px', marginBottom: '20px', color: '#333' }}>Personal Info</h3>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                <p><strong>Address:</strong> {user.address?.village}, {user.address?.taluka}, {user.address?.district}, {user.address?.state} - {user.address?.pincode}</p>
                                {user.role === 'farmer' ? (
                                    <p><strong>Land Size:</strong> {user.landSize} Acres</p>
                                ) : (
                                    <p><strong>Shop Name:</strong> {user.shopName || 'Not Provided'}</p>
                                )}
                            </div>
                        </div>
                        ...
                        {user.role === 'owner' && user.tools && user.tools.length > 0 && (
                            <div style={{ background: 'white', borderRadius: '20px', padding: '25px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                                <h3 style={{ borderBottom: '2px solid #f0f0f0', paddingBottom: '10px', marginBottom: '20px', color: '#333' }}>Your Tools</h3>
