...
export default function Hire() {
    ...
    const fetchOwners = async (toolName, bookingDate) => {
        try {
            const res = await fetch(
                    `http://127.0.0.1:5000/owners?tool=${encodeURIComponent(toolName)}&date=${bookingDate}`
                                 );
    ...
    useEffect(() => {
        ...
        if (location.state?.ownerName) {
            ...
            setOwners([ownerData]);
            setSelectedTool({ name_mr: location.state.tool, name_en: location.state.tool });
            setViewOwners(true);
        }
    ...
    return (
        <div className="container" style={{ padding: '5rem 1rem' }}>
            <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                <h1 style={{ fontSize: '2.5rem', color: 'var(--primary-green)', marginBottom: '1rem', fontWeight: 800 }}>{'अवजारे भाड्याने घ्या'}</h1>
                <p style={{ fontSize: '1.2rem', color: 'var(--text-dark)', maxWidth: '600px', margin: '0 auto' }}>
                    {'परवडणाऱ्या दरात भाड्याने मिळणाऱ्या आमच्या प्रीमियम कृषी अवजारांची यादी एक्सप्लोर करा.'}
                </p>
            </div>
            ...
            {viewOwners && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h2 style={{ color: 'var(--primary-green)', marginBottom: '0.5rem' }}>{selectedTool?.name_mr} साठी मालक</h2>
                        <p style={{ color: '#666', marginBottom: '2rem' }}>आमच्या जोडलेल्या शेतकऱ्यांकडून उपलब्ध भाड्याचे पर्याय दाखवत आहे.</p>
                                ...
                                <input
                                    ...
                                    onChange={async (e) => {
                                         ...
                                         const date = e.target.value;
                                            ...
                                            if (date && selectedTool) {

                                                 console.log("SELECTED DATE:", date);

                                                await fetchOwners(selectedTool.name_mr, date);
                                              }
                                    }}
                                    style={{
                                        width: '100%',
                        ...
                        {owners.length > 0 ? (
                                    ...
                                    <div key={idx} style={{
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '1.2rem', color: 'var(--text-dark)' }}>
                                                {f.owner_name || 'अनामित मालक'}
                                            </h4>
                                            <p style={{ margin: 0, fontSize: '0.95rem', color: '#666' }}>
                                                {f.mobile || f.mobile || 'माहिती दिलेली नाही.'}
                                        ...
                                        <div style={{ textAlign: 'right', marginLeft: '1.5rem' }}>
                                            <span style={{
                                                ...
                                                ₹{getStaticPrice(selectedTool?.name_mr)}
                                            </span>
                                            <button
                            ...
                            <div style={{ textAlign: 'center', padding: '3rem' }}>
                                ...
                                <p style={{ color: '#999' }}>या अवजारासाठी सध्या कोणतेही मालक आढळले नाहीत.</p>
                            </div>
                        )}
                    </div>
                </div>
